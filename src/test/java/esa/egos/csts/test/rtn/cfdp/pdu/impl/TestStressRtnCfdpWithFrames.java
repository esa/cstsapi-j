...
public class TestStressRtnCfdpWithFrames {
     ...
     public void implementRtnCfdpPduDataTransfer(int repetitions, int nPdus, int fullPduSize) {
          ...
          try {
               ...
               CSTS_LOG.CSTS_API_LOGGER
                         .info("Total full PDUs size: " + repetitions * nPdus * fullPduSize / (1000 * 1000)
                                   + " MByte, rate: " + rate / 1_000_000 + " Mbit/s");
               CSTS_LOG.CSTS_API_LOGGER.info("Total reduced PDUs size: " + nPdus * 18 / (1000 * 1000) + " MByte, rate: "
                         + rateReduced / 1000_000 + " Mbit/s");
               CSTS_LOG.CSTS_API_LOGGER.info(
                         "Full PDUs: " + repetitions * nPdus + " rate: " + repetitions * nPdus / timeSpan + " Units/s");
     ...
     private void doDataTransfer(RtnCfdpPduSiUser userSi, RtnCfdpPduSiProvider providerSi, List<CcsdsTmFrame> frames,
               int repetitions, boolean withTime) throws InterruptedException {
          ...
          CfdpPduExtractAndTransfer extractAndTransfer = new CfdpPduExtractAndTransfer(packAndTransfer, frameConfigData,
                    packetConfigData);
