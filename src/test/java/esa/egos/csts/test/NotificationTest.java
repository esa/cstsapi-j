...
public class NotificationTest {
	...
	public void test() {
		...
		INotification notification = (INotification) siUser.getProcedure(pidUser);
		notification.setListOfEvents(new ListOfParameters(new ProcedureType(OIDs.informationQuery)));
		try {
			
			IStart start;
			start = siUser.createOperation(IStart.class);
			...
			IStop stop;
			stop = siUser.createOperation(IStop.class);
			siUser.initiateOpInvoke(stop, 352);
	...
	private void setProceduresAndConfigure(IServiceInstance si) throws ApiException {
		...
		if(si.getRole() == AppRole.USER)
			secondaryProcedure = si.createProcedure(NotificationUser.class, version);
		else
			secondaryProcedure = si.createProcedure(NotificationProvider.class, version);
