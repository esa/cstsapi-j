...
import esa.egos.csts.api.enums.AssocState;
import esa.egos.csts.api.enums.BindRole;
...
import esa.egos.csts.api.proxy.IProxy;
import esa.egos.csts.api.proxy.ILocator;
import esa.egos.csts.api.proxy.ServiceType;
import esa.egos.csts.api.proxy.impl.EE_APIPX_BinderPxy;
                                                    ...
                                                    IServiceInstance pclientIf) throws ApiException
    {
        ...
        if (sleAppRole == AppRole.PROVIDER)
        {
        	...
        	for(ServerType serverType: pSrvTypeList){
        		if(serverType.getServerId().equals(srvType.getApplicationIdentifierOID().toString()))
        			pSrvType = serverType;
        	}
            if (pSrvType == null)
            {
