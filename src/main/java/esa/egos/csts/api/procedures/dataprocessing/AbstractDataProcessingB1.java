...
public abstract class AbstractDataProcessingB1 extends AbstractDataProcessing {
	...
	protected EmbeddedData encodeNotifyInvocationExtension(boolean svcProductionStatusChange) {
		...
		if (getLastProcessedDataUnitId() < 0) {
			...
			dataUnitLastProcessed.setDataUnitId(new DataUnitId(getLastProcessedDataUnitId()));
			...
			if (getLastProcessedDataUnitId() < 0) {
				...
				dataUnitId.setDataUnitId(new DataUnitId(getLastProcessedDataUnitId()));
				dataUnitId.setDataProcessingStopTime(getLastProcessedDataUnitTime().encode(new b1.ccsds.csts.common.types.Time()));
		...
		if (svcProductionStatusChange) {
			...
			productionStatus.setAnyOtherEvent(getServiceInstance().getProductionStatus().encode(new b1.ccsds.csts.common.types.ProductionStatus()));
		}
	...
	protected void decodeNotifyInvocationExtension(Extension extension) {
		if (extension.isUsed() && extension.getEmbeddedData().getOid().equals(OIDs.dpNotifyInvocExt)) {
			...
			if (dataUnitIdLastProcessed.getNoDataProcessed() != null) {
				...
				setLastProcessedDataUnitId(dataUnitLastProcessed.getDataUnitId().longValue());
			...
			if (dataUnitIdLastOk.getNoSuccessfulProcessing() != null) {
				...
				setLastProcessedDataUnitId(dataUnitIdLastOk.getDataUnitId().getDataUnitId().longValue());
