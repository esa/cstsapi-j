...
public abstract class AbstractAssociationControl extends AbstractProcedure implements IAssociationControl {
	...
	private final ProcedureType type = new ProcedureType(OIDs.associationControl);

	private IServiceInstanceInternal internalServiceInstance;

	/**
	 * Constructor of Association control. Do not set the state here before it is
	 * initialised.
	 */
	...
	public byte[] encodeOperation(IOperation operation, boolean isInvoke) throws ApiException, IOException {
		...
		if (IBind.class.isAssignableFrom(operation.getClass())) {
			if (isInvoke) {
				...
				BindInvocation obj = bind.encodeBindInvocation();
				...
				IBind bind = (IBind) operation;
				BindReturn obj = bind.encodeBindReturn();
	...
	protected void encodeUnbindReturnOp(IOperation operation, UnbindReturn obj) {
		...
		if (unbind != null) {
			...
			obj.setPerformerCredentials(unbind.getPerformerCredentials().encode());
	...
	*/
	...
	protected void encodeBindInvokeOp(IOperation operation, BindInvocation obj) {
		...
		if (bind != null) {
			...
			// the service type
			obj.setServiceType(new ServiceType(bind.getServiceType().getOid().getOidArray()));
			...
			obj.setBindInvocationExtension(ExtensionUtils.nonUsedExtension());
		}
	}
	*/
	...
	protected void createAssoc() throws ApiException {
		LOG.finest("Creating association for service instance "
				+ getServiceInstanceInternal().getServiceInstanceIdentifier().toString());
		...
		String protId = getServiceInstance().getApi()
				.getProtocolId(getServiceInstanceInternal().getResponderPortIdentifier());
		...
		// ISrvProxyInform ppServiceInstance = (ISrvProxyInform)
		// getServiceInstanceInternal();

		try {
			pi = af.createAssociation(ISrvProxyInitiate.class, getServiceInstanceInternal().getServiceType(),
					getServiceInstanceInternal());
	...
	/**
	 * Processes the PEER-ABORT invocation received from the proxy or application.
	 * The function also invokes state-processing, which performs an internal
	 * cleanup.
	 */
	protected Result peerAbortInvocation(IPeerAbort poperation, AbortOriginator originator) {
		Component cp = Component.CP_application;
		if (originator == AbortOriginator.AO_peer || originator == AbortOriginator.AO_proxy) {
