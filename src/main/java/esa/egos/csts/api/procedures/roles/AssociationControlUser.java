...
public class AssociationControlUser extends AbstractAssociationControl {
	...
	/**
	 * The meaning of the booleans is as follows: case 1: originatorIsNetwork = true
	 * -> from Proxy case 2: originatorIsNetwork = false and isInvoke = true -> from
	 * Application case 3: originatorIsNetwork = false and isInvoke = false ->
	 * internal event
	 */
	@Override
	protected Result doStateProcessing(IOperation operation, boolean isInvoke, boolean originatorIsNetwork) {
		...
		if (!originatorIsNetwork) {
			...
			if (IBind.class.isAssignableFrom(operation.getClass()) && isInvoke) {
				...
				LOG.fine(
						"Protocol error, event: State transition from " + state.name() + "on " + IBind.class.getName());
			...
			// case UnbindInv:
				...
				LOG.fine("Protocol error, event: State transition from " + state.name() + "on "
						+ IUnbind.class.getName());
				return Result.SLE_E_PROTOCOL;
			}
			// case PeerAbortInv:
