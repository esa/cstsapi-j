...
public class Get extends AbstractConfirmedOperation implements IGet {
	...
	@Override
	public BerType encodeGetInvocation() {
		GetInvocation getInvocation = new GetInvocation();
		getInvocation.setStandardInvocationHeader(encodeStandardInvocationHeader());
		getInvocation.setListOfParameters(listOfParameters.encode(new b1.ccsds.csts.common.types.ListOfParametersEvents()));
		getInvocation.setGetInvocationExtension(invocationExtension.encode(new b1.ccsds.csts.common.types.Extended()));
	...
	private EmbeddedData encodeGetPosReturnExt() {
		...
		for (QualifiedParameter param : qualifiedParameters) {
			qualifiedParametersSequence.getQualifiedParameter().add(param.encode(new b1.ccsds.csts.common.types.QualifiedParameter()));
		}
	...
	private EmbeddedData encodeGetDiagnosticExt() {
		...
		getDiagnosticExt.setCommon(listOfParametersDiagnostics.encode(new b1.ccsds.csts.common.types.ListOfParamEventsDiagnostics()));
		// encode with a resizable output stream and an initial capacity of 128 bytes
		try (ReverseByteArrayOutputStream os = new ReverseByteArrayOutputStream(128, true)) {
