...
import b2.ccsds.csts.common.types.InvokeId;
...
public abstract class AbstractOperation implements IOperation {
	 ...
	 */
	protected b2.ccsds.csts.common.types.Credentials getEmptyCredentials() {
		b2.ccsds.csts.common.types.Credentials cred = new b2.ccsds.csts.common.types.Credentials();
	...
	protected final  StandardInvocationHeader encodeStandardInvocationHeader() {
		...
		StandardInvocationHeader header = 
				 new StandardInvocationHeader();
		header.setInvokeId(new InvokeId(getInvokeIdentifier()));
		 Credentials credentials = (Credentials)getInvokerCredentials();
		if (getInvokerCredentials() != null) {
			header.setInvokerCredentials(credentials.encode(new b2.ccsds.csts.common.types.Credentials()));
		} else {
			header.setInvokerCredentials(getEmptyCredentials());
		}
		header.setProcedureName(getProcedureInstanceIdentifier().encode(new b2.ccsds.csts.common.types.ProcedureName()));
	...
	protected final void decodeStandardInvocationHeader(StandardInvocationHeader standardInvocationHeader) {
		...
		procedureInstanceIdentifier = ProcedureInstanceIdentifier
				.decode(standardInvocationHeader.getProcedureName());
	}
