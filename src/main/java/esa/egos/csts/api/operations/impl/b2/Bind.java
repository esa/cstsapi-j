...
public class Bind extends AbstractConfirmedOperation implements IBind {
	...
	public String print(int i) {
		...
		sb.append("Version                        : ").append(this.serviceVersion).append('\n');
	...
	public BerType encodeBindInvocation() {
		...
		bindInvocation.setInitiatorIdentifier(new AuthorityIdentifier(
		...
		// the service type
		bindInvocation.setServiceType(serviceType.encodeB2());

		// the version number
		bindInvocation.setVersionNumber(new b2.ccsds.csts.association.control.types.VersionNumber(
				getServiceVersion()));

		// the service instance identifier
		bindInvocation.setServiceInstanceIdentifier(getServiceInstanceIdentifier().encode(new b2.ccsds.csts.service.instance.id.ServiceInstanceIdentifier()));

		// the extension, if available
		bindInvocation.setBindInvocationExtension(invocationExtension.encode(new b2.ccsds.csts.common.types.Extended()));
	...
	public void decodeBindInvocation(BerType bindInvocation) {
		...
		BindInvocation bindInvocation2 = (BindInvocation) bindInvocation;

		decodeStandardInvocationHeader(bindInvocation2.getStandardInvocationHeader());
		initiatorIdentifier = CSTSUtils.decodeString(bindInvocation2.getInitiatorIdentifier().value);
		responderPortIdentifier = CSTSUtils.decodeString(bindInvocation2.getResponderPortIdentifier().value);
		serviceType = ServiceType.decode(bindInvocation2.getServiceType());
		serviceVersion = bindInvocation2.getVersionNumber().intValue();

		try {
			setServiceInstanceIdentifier(
					ServiceInstanceIdentifier.decode(bindInvocation2.getServiceInstanceIdentifier()));
		} catch (ConfigException e) {
			e.printStackTrace();
		}

		invocationExtension = Extension.decode(bindInvocation2.getBindInvocationExtension());
