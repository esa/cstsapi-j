...
import b2.ccsds.csts.common.types.StandardReturnHeader.Result;
import b2.ccsds.csts.common.types.StandardReturnHeader.Result.Negative;
...
public abstract class AbstractConfirmedOperation extends AbstractOperation implements IConfirmedOperation {
	...
	protected final <T extends StandardReturnHeader> T encodeStandardReturnHeader(Class<T> cls) {
		...
		if (getPerformerCredentials() != null) {
			header.setPerformerCredentials(credentials.encode(new b2.ccsds.csts.common.types.Credentials()));
		} else {
	...
	private Result encodeResult() {
		...
		if (operationResult == OperationResult.POSITIVE) {
			...
			result.setPositive(returnExtension.encode(new b2.ccsds.csts.common.types.Extended()));
		} else {
	...
	private Negative encodeNegativeReturn() {
		...
		negative.setDiagnostic(diagnostic.encode(new b2.ccsds.csts.common.types.Diagnostic()));
		negative.setNegExtension(returnExtension.encode(new b2.ccsds.csts.common.types.Extended()));
		return negative;
	}
